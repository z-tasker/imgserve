...
def pad_experiment(directory: Path) -> None:
    ...
    for colorgram in colorgrams:
        dimensions = {
            dim.split("=")[0]: dim.split("=")[1] for dim in colorgram.split("|")
        }
        all_queries.add(dimensions["query"])
        all_regions.add(dimensions["region"])

    region_first = (
        True if str(list(directory.iterdir())[0].name).startswith("region") else False
    )
    for query in all_queries:
        for region in all_regions:
            if region_first:
                colorgram_path = directory.joinpath(
                    f"region={region}|query={query}.png"
                )
            else:
                colorgram_path = directory.joinpath(
                    f"query={query}|region={region}.png"
                )
